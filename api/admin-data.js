...
const { db, upsertCustomer, normalizePhone, auditEvent } = require('./_db');
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    if(req.method==='POST' && action==='customer'){
      ...
      return json(res,200,{status:'success',customer});
    }
    ...
    if(req.method==='POST' && action==='inspection'){
      const b=req.body||{};
      if(!s(b.customer_name||b.customerName,120)||!s(b.vehicle,300))return json(res,400,{error:'Customer name and vehicle are required'});
      const customer=await upsertCustomer(supabase,{name:b.customer_name||b.customerName,phone:b.phone||'No phone',email:b.email,vehicle:b.vehicle,mileage:b.mileage},shop.id).catch(()=>null);
      const row=withShopId({
        customer_id:customer?.id||null,customer_name:s(b.customer_name||b.customerName,120),phone:s(b.phone,40)||null,email:s(b.email,200)||null,vehicle:s(b.vehicle,300),
      ...
      for(const key of ['brakes','tires','suspension','fluids','battery','lights','wipers','filters','leaks']){
        row[`${key}_status`]=s(b[`${key}_status`]??b[`${key}Status`],50)||'Monitor';
        row[`${key}_notes`]=s(b[`${key}_notes`]??b[`${key}Notes`],2000)||null;
      }
